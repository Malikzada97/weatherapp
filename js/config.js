...
export const config = {
  ...
  OPENWEATHER_API_KEY: getEnvVar('OPENWEATHER_API_KEY', 'dacc36845acc003ff207a6e657a47718'),
  ...
  WEATHER_EFFECTS: {
    RAIN: {
      enabled: getEnvVar('WEATHER_EFFECTS_RAIN_ENABLED', 'true') === 'true',
      intensity: parseFloat(getEnvVar('WEATHER_EFFECTS_RAIN_INTENSITY', '0.5'))
    },
    SNOW: {
      enabled: getEnvVar('WEATHER_EFFECTS_SNOW_ENABLED', 'true') === 'true',
      intensity: parseFloat(getEnvVar('WEATHER_EFFECTS_SNOW_INTENSITY', '0.3'))
    },
    CLOUDS: {
      enabled: getEnvVar('WEATHER_EFFECTS_CLOUDS_ENABLED', 'true') === 'true',
...
export const getApiKey = () => {
  ...
  if (!apiKey || apiKey === 'your_api_key_here' || apiKey === 'dacc36845acc003ff207a6e657a47718') {
    console.warn('⚠️  Please configure your OpenWeatherMap API key in the .env file or config.js');
